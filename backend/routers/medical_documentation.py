...
async def get_medical_documentation(user_id: int = Query(None), db: Session = Depends(get_db)):
    ...
    medical_documentation = db.query(models.MedicalDocumentation).filter(models.MedicalDocumentation.patient_id == user_id).options(joinedload(models.MedicalDocumentation.entries)).first()
    if medical_documentation is None:
